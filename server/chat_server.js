/**
 * Created by lichun on 2018/3/27.
 */
...
function joinRoom(socket, room) {
	...
	socket.broadcast.to(room).emit('message:system',{      //让房间里的其他用户知道有新用户进入
	...
	if(usersInRoom.length > 1){     //如果不止一个用户在这个房间内,汇总一下都有谁
		var usersInRoomSummary = '当前房间: ' + room + ' 共有 ' + usersInRoom.length + ' 人在线';
		...
		usersInRoomSummary += '.';
		socket.emit('message:system', {text: usersInRoomSummary}); //将房间里其他用户的汇总发送给这个用户
	}
}
...
function handleMessageBroadcasting(socket, nickNames) {
	socket.on('message:userInput', function (message) {
		socket.broadcast.to(message.room).emit('message:userInput',{
